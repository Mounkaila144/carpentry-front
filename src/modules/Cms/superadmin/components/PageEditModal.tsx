...
import type { Page, UpdatePagePayload, PageStatus, PageTemplate } from '../../types'
...
const PageEditModal = ({ open, onClose, onSuccess, page }: PageEditModalProps) => {
  const { tenantId } = useTenant()

  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const [formData, setFormData] = useState<UpdatePagePayload>({
    title: '',
    slug: '',
    content: '',
    excerpt: '',
  ...
  const handleFieldChange = (field: keyof UpdatePagePayload, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }))
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await pageService.updatePage(page.id, formData, tenantId)
    ...
    <Dialog open={open} onClose={handleClose} maxWidth='md' fullWidth>
      ...
      <DialogActions>
        ...
        <Button onClick={handleSubmit} variant='contained' disabled={!isFormValid() || loading}>
          {loading ? <CircularProgress size={20} /> : 'Update Page'}
        </Button>
      </DialogActions>
    </Dialog>
  )
